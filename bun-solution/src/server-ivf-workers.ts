/**
 * Server with IVF + Worker Threads - Approach 3 (Best Combined)
 * IVF reduces per-query work from 3M to ~60K vectors.
 * Workers keep the event loop free for I/O under concurrency.
 */
...
// Create worker pool using IVF workers
const pool = new WorkerPool(new URL("./ivf-search-worker.ts", import.meta.url).pathname, NUM_WORKERS);
...
console.log(`IVF+Workers server ready (${NUM_WORKERS} workers)`);
...
console.log(`IVF+Workers server running on port ${PORT}`);
